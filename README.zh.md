...
<p align="center">
  ...
  按住快捷键说话，AI 润色后的文字以<em>你选定的风格</em>，流式落到当前光标。
</p>
...
两个让 OpenLess 日常体验完全不同的功能：

- 🎨 **风格市场（Style Pack Marketplace）。** OpenLess 不再只有一种写死的「润色」语气。你可以自定义带专属系统提示词的**风格包**，用快捷键随时切换，还能**一键安装社区里别人分享的风格包**，或把自己的风格包发布出去共享。当风格被调到贴合你的真实场景（冷启动邮件、commit message、小红书文案、正式报告、团队语气）时，输出不只是更干净——**效果会明显更好**，因为模型终于在按你真正想要的方式写字。
- ⚡ **流式输出（Streaming insertion）。** 润色后的文字现在会**逐字**流式写入光标，而不是让你干等整段结果落地。体感延迟大幅下降，口述快得几乎跟得上你的思路；当某个应用无法接收流式按键时，会自动回退为一次性粘贴。
...
**风格包**就是一个有名字、有专属系统提示词的输出风格。你不必被一种内置「润色」语气框住——可以精确地塑造语音输出的样子，而且风格越贴近你的真实场景，效果越好。

- **自定义创作。** 在**风格**页新建风格包，写一段自定义系统提示词（比如「极简工程 commit message」「温暖的客服回复」「带 emoji 的小红书文案」），用快捷键切换当前生效的风格包。
- **从社区安装。** 打开**市场**，浏览、搜索并一键安装别人分享的风格包，给好用的点赞。
- **发布自己的。** 在 设置 → 市场 用 GitHub 身份登录后，从风格页点 **发布到市场**。上传内容会先经过审核，通过后才公开。

市场由 OpenLess 自建的、带审核的后端提供，所以目录是经过策展的，而不是任人堆砌。
...
- 🎨 **风格市场（Style Pack Marketplace）** — 在 App 内市场浏览、安装、点赞社区**风格包**，也可发布自己的（每个风格包带独立系统提示词，可用快捷键切换）。由带审核的市场后端提供，上传内容审核通过后才公开。
- ⚡ **流式输出（Streaming insertion）** — 润色文字逐字写入光标以降低体感延迟，并在不满足条件时自动回退为一次性粘贴；可在 设置 → 录音 开关。
- **切换式 + 按住说话** 双模式录音，外加 **MediaPlayPause 触发**，让有线耳机线控也能开始/停止录音；任意阶段按 `Esc` 都能取消（包括润色 / 插入中）。
...
- 主窗口按「概览 / 历史 / 词典 / 风格 / 市场 / 设置」组织；托盘图标常驻；浮动状态胶囊会跟随你正在输入的屏幕显示（多显示器）。
- **本地模型管理** — 在设置中管理本地 ASR 模型的磁盘存储。
