...
impl SettingsWriter for Coordinator {
    ...
    fn refresh_opencode_agent_hotkey(&self) {
        self.update_opencode_agent_hotkey_binding();
    }
}
...
impl<T: SettingsWriter + ?Sized> SettingsWriter for Arc<T> {
    ...
    fn refresh_opencode_agent_hotkey(&self) {
        (**self).refresh_opencode_agent_hotkey();
    }
}
...
pub(crate) fn persist_settings<T: SettingsWriter>(
    ...
    let opencode_agent_changed = previous.opencode_agent_enabled != prefs.opencode_agent_enabled
        || previous.opencode_agent_voice_hotkey != prefs.opencode_agent_voice_hotkey;
    ...
    if opencode_agent_changed {
        coord.refresh_opencode_agent_hotkey();
    }
    Ok(())
}
