...
pub fn set_qa_hotkey(
    ...
    if let Some(binding) = binding.as_ref() {
        ...
        if let Some(opencode) = prefs.opencode_agent_voice_hotkey.as_ref() {
            reject_qa_opencode_hotkey_overlap(binding, opencode)?;
        }
    }
...
/// 用户点 ✕ / 按 Esc 关 OpenCode 浮窗。
#[tauri::command]
pub fn opencode_window_dismiss(coord: CoordinatorState<'_>) {
    coord.opencode_window_dismiss();
}

/// 前端按内容测高后回传高度，后端 clamp + bottom-anchored 重新摆放浮窗。
#[tauri::command]
pub fn opencode_window_resize(coord: CoordinatorState<'_>, height: f64) {
    coord.opencode_window_resize(height);
}

/// 内联审批卡的 Approve / Deny 回执。token 关联到等待中的拦截动作。
///
/// 安全：审批 UI 渲染在 opencode 窗口（OpenCodePanel），故仅允许该窗口提交，
/// 拦截 main / capsule / qa / glow 等其它窗口伪造审批 —— 把可调用窗口从 5 个收紧到 1 个。
#[tauri::command]
pub fn opencode_approve(
    window: Window,
    coord: CoordinatorState<'_>,
    token: String,
    approved: bool,
    ...
    if window.label() != "opencode" {
        return Err("approval can only be submitted from the OpenCode window".to_string());
    }
    coord.opencode_approve(&token, approved);
    Ok(())
}
