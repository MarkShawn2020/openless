...
mod tests {
    ...
    struct FakeSettingsWriter {
        ...
        opencode_agent_refreshes: Mutex<u32>,
    }
    ...
    impl SettingsWriter for FakeSettingsWriter {
        ...
        fn refresh_opencode_agent_hotkey(&self) {
            *self.opencode_agent_refreshes.lock().unwrap() += 1;
        }
    }
    ...
    fn persist_settings_refreshes_changed_hotkey_pipelines() {
        ...
        *writer.saved.lock().unwrap() = Some(previous);
        let prefs = UserPreferences {
            ...
            opencode_agent_voice_hotkey: Some(ShortcutBinding {
        ...
        assert_eq!(*writer.opencode_agent_refreshes.lock().unwrap(), 1);
    }

    #[test]
    fn persist_settings_rejects_opencode_dictation_overlap() {
        let writer = FakeSettingsWriter::default();
        ...
        let prefs = UserPreferences {
            ...
            opencode_agent_voice_hotkey: Some(binding),
            ..Default::default()
        };

        assert_eq!(
            persist_settings(&writer, prefs),
            Err("OpenCode 快捷键不能和听写快捷键相同".into())
        );
    ...
    fn persist_settings_syncs_active_asr_provider_without_hotkey_refresh() {
        ...
        *writer.saved.lock().unwrap() = Some(previous.clone());
        ...
        assert_eq!(*writer.opencode_agent_refreshes.lock().unwrap(), 0);
    }
