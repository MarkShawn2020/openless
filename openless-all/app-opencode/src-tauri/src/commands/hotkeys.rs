...
pub fn set_dictation_hotkey(
    ...
    if let Some(opencode) = prefs.opencode_agent_voice_hotkey.as_ref() {
        reject_dictation_opencode_hotkey_overlap(&binding, opencode)?;
    }
...
pub fn set_translation_hotkey(
    ...
    if let Some(opencode) = previous.opencode_agent_voice_hotkey.as_ref() {
        reject_translation_opencode_hotkey_overlap(&binding, opencode)?;
    }
...
pub fn set_switch_style_hotkey(
    ...
    if let Some(binding) = binding.as_ref() {
        ...
        if let Some(opencode) = prefs.opencode_agent_voice_hotkey.as_ref() {
            reject_opencode_switch_style_hotkey_overlap(opencode, binding)?;
        }
    }
...
pub fn set_open_app_hotkey(
    ...
    if let Some(binding) = binding.as_ref() {
        ...
        if let Some(opencode) = prefs.opencode_agent_voice_hotkey.as_ref() {
            reject_opencode_open_app_hotkey_overlap(opencode, binding)?;
        }
    }
...
pub fn set_combo_hotkey(coord: CoordinatorState<'_>, binding: ComboBinding) -> Result<(), String> {
    ...
    if let Some(opencode) = prefs.opencode_agent_voice_hotkey.as_ref() {
        reject_dictation_opencode_hotkey_overlap(&shortcut, opencode)?;
    }
...
pub(crate) fn reject_hotkey_collisions(prefs: &UserPreferences) -> Result<(), String> {
    ...
    let opencode = prefs.opencode_agent_voice_hotkey.as_ref();
    if let Some(qa_hotkey) = prefs.qa_hotkey.as_ref() {
        ...
        if let Some(opencode) = opencode {
            reject_qa_opencode_hotkey_overlap(qa_hotkey, opencode)?;
        }
    ...
    if let Some(opencode) = opencode {
        reject_dictation_opencode_hotkey_overlap(&prefs.dictation_hotkey, opencode)?;
        reject_translation_opencode_hotkey_overlap(&prefs.translation_hotkey, opencode)?;
    }
    if let Some(switch_style) = switch_style {
        ...
        if let Some(opencode) = opencode {
            reject_opencode_switch_style_hotkey_overlap(opencode, switch_style)?;
        }
    }
    if let Some(open_app) = open_app {
        ...
        if let Some(opencode) = opencode {
            reject_opencode_open_app_hotkey_overlap(opencode, open_app)?;
        }
    }
...
fn reject_dictation_opencode_hotkey_overlap(
    dictation: &ShortcutBinding,
    opencode: &ShortcutBinding,
) -> Result<(), String> {
    reject_hotkey_overlap(
        dictation,
        opencode,
        "OpenCode 快捷键不能和听写快捷键相同",
    )
}
...
pub(crate) fn reject_qa_opencode_hotkey_overlap(
    qa: &ShortcutBinding,
    opencode: &ShortcutBinding,
) -> Result<(), String> {
    reject_hotkey_overlap(
...
fn reject_translation_opencode_hotkey_overlap(
    translation: &ShortcutBinding,
    opencode: &ShortcutBinding,
) -> Result<(), String> {
    reject_hotkey_overlap(
...
fn reject_opencode_switch_style_hotkey_overlap(
    opencode: &ShortcutBinding,
    switch_style: &ShortcutBinding,
) -> Result<(), String> {
    reject_hotkey_overlap(
        opencode,
        switch_style,
        "OpenCode 快捷键不能和切换风格快捷键相同",
    )
}

fn reject_opencode_open_app_hotkey_overlap(
    opencode: &ShortcutBinding,
    open_app: &ShortcutBinding,
) -> Result<(), String> {
    reject_hotkey_overlap(
...
mod tests {
    ...
    /// 锁定碰撞矩阵：每个动作键与 OpenCode 键相同都必须被 reject_hotkey_collisions
    /// 拒绝。5 个快捷 setter（set_dictation/translation/switch_style/open_app/qa_hotkey）
    /// 此前漏校验 opencode_agent_voice_hotkey，已接入对应的 opencode 校验。
    #[test]
    fn each_action_hotkey_collides_with_opencode() {
        ...
        let mut prefs = UserPreferences {
            ...
            opencode_agent_voice_hotkey: Some(lc.clone()),
            ..Default::default()
        };
        // 基线全不同 → 通过。
