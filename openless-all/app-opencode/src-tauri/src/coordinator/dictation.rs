...
/// OpenCode 浮窗的 Tauri 事件名（前端 OpenCodePanel 订阅）。
const OPENCODE_EVENT: &str = "opencode:event";

/// OpenCode 内联审批：等待用户决断的 token → oneshot sender 注册表。
///
/// 无头 `opencode -p` 没有 mid-run 的 permission 通道，所以护栏拦截发生在
/// 「整轮跑完、护栏 deny 生效」之后。这个注册表是审批 UI 的实回路：
/// 后端发 `approval` 事件后把一个 oneshot 接收端挂在这里，等前端 `opencode_approve`
/// 命令按 token 解析出用户决断（true=Approve / false=Deny）。
static OPENCODE_APPROVALS: std::sync::OnceLock<
    std::sync::Mutex<std::collections::HashMap<String, tokio::sync::oneshot::Sender<bool>>>,
> = std::sync::OnceLock::new();

fn opencode_approvals(
) -> &'static std::sync::Mutex<std::collections::HashMap<String, tokio::sync::oneshot::Sender<bool>>>
{
    OPENCODE_APPROVALS.get_or_init(|| std::sync::Mutex::new(std::collections::HashMap::new()))
}

/// 前端 `opencode_approve` 命令调到这里：按 token 解析等待中的审批。
/// token 不存在（已超时 / 已解析）时静默忽略。
pub(super) fn resolve_opencode_approval(token: &str, approved: bool) {
    let sender = opencode_approvals()
        .lock()
        .ok()
        .and_then(|mut m| m.remove(token));
    if let Some(tx) = sender {
        let _ = tx.send(approved);
        log::info!("[opencode] 审批 token={token} approved={approved}");
    } else {
        log::info!("[opencode] 审批 token={token} 已失效（超时/重复）");
    }
}

/// 往 OpenCode 浮窗发一条事件（macOS only；前端按 `kind` 渲染聊天结构）。
fn emit_opencode(inner: &Arc<Inner>, payload: serde_json::Value) {
    if let Some(app) = inner.app.lock().clone() {
        let _ = app.emit_to("opencode", OPENCODE_EVENT, payload);
    }
}
...
/// OpenCode 收尾：把转写当作指令交给无头 OpenCode，结果以胶囊展示（不插入到光标）。
async fn run_voice_agent_transcript(
    ...
    emit_capsule(
        ...
        0.0,
        elapsed,
        Some("OpenCode 处理中…".to_string()),
        None,
    );

    // 聊天浮窗：显示窗口 + 落用户气泡（语音指令转写）。macOS only（helper 内部 gating）。
    if let Some(app) = inner.app.lock().clone() {
        crate::show_opencode_window(&app);
        // 全屏彩虹描边已在按下键时（handle_opencode_pressed）点亮，这里不重复。
    }
    // 连续对话：浮窗里已有进行中的会话 → 本轮 `opencode --continue` 续上下文；否则是新会话（fresh）。
    ...
    if let Some(dir) = &cwd {
        if let Some(sha) = crate::opencode_agent::create_git_snapshot(dir) {
            log::info!("[opencode] 运行前 git 快照 {sha}（git stash apply 可回滚）");
        }
    }

    // 钳制：语音 → shell 这条全自动路径禁止 bypassPermissions 绕过护栏（无人审、动手即生效）。
    // 即便用户在偏好里设了 bypass，这里也降级为 acceptEdits（仍带 deny 护栏）。
    let mode = match opencode_agent_mode_from_pref(&prefs.opencode_agent_permission_mode) {
        crate::opencode_agent::OpenCodeAgentPermissionMode::BypassPermissions => {
            log::warn!(
                "[opencode] 语音 Agent 路径禁止 bypassPermissions，已降级为 acceptEdits（保留护栏）"
            );
            crate::opencode_agent::OpenCodeAgentPermissionMode::AcceptEdits
        }
        other => other,
    };
    let model = prefs
    ...
    let prompt = crate::opencode_agent::autonomous_prompt(&transcript);

    // 第一轮：默认护栏（高风险全 deny）。运行后若检测到护栏拦截，弹审批卡；
    // 用户 Approve 则在第二轮把该高风险模式从 deny 移除 + 加进 allowed，重跑一次。
    let outcome = run_opencode_once(
        inner,
        &prompt,
        cwd.as_deref(),
        mode,
    ...
    let final_outcome = match maybe_request_approval(inner, &outcome).await {
        Some(approved_pattern) => {
            log::info!("[opencode] 审批通过，放行高风险模式后重跑：{approved_pattern}");
            run_opencode_once(
                inner,
                &prompt,
                cwd.as_deref(),
    ...
    if let Some(app) = inner.app.lock().clone() {
        crate::hide_opencode_glow(&app);
    }

    match final_outcome {
        OpenCodeOutcome::Done { text, cost_usd } => {
            let text = text.trim().to_string();
            if text.is_empty() {
                let msg = "OpenCode 无结果（确认已登录 opencode 且额度充足）".to_string();
                emit_opencode(
                    inner,
            ...
            log::info!("[coord] OpenCode 语音：返回 {} 字", text.chars().count());
            emit_opencode(
                inner,
        ...
        OpenCodeOutcome::Failed { message } => {
            log::warn!("[coord] OpenCode 语音失败: {message}");
            emit_opencode(
                inner,
        ...
        OpenCodeOutcome::Cancelled => {
            log::info!("[coord] OpenCode 语音已取消");
            emit_opencode(inner, serde_json::json!({ "kind": "cancelled" }));
...
/// 跑一轮无头 OpenCode（「放行 + 护栏」），把 Delta/ToolUse 实时 stream 到聊天浮窗，
/// 终局收敛为 [`OpenCodeOutcome`]。`extra_allow_patterns` 为审批通过后放行的
/// 高风险子串（如 "git push --force"）：从 bash deny 规则中剔除。
async fn run_opencode_once(
    inner: &Arc<Inner>,
    prompt: &str,
    cwd: Option<&std::path::Path>,
    mode: crate::opencode_agent::OpenCodeAgentPermissionMode,
    model: Option<&str>,
    extra_allow_patterns: &[String],
    continue_session: bool,
) -> OpenCodeOutcome {
    // 生成 opencode 格式的权限配置 JSON（与 Claude Code 的 settings 格式不同）。
    // opencode 用 `permission`（单数）+ 工具名为 key，支持对象语法的细粒度模式匹配。
    let permission_json = crate::opencode_agent::guard::build_opencode_permission_json(
        mode.as_cli_arg(),
        extra_allow_patterns,
    );
    let config_path = std::env::temp_dir().join(format!(
        "openless-opencode-guard-{}.json",
        uuid::Uuid::new_v4()
    ));
    // fail-closed：序列化或写入失败时立即中止，绝不在「无护栏」下把无效路径交给 opencode。
    let config_bytes = match serde_json::to_vec_pretty(&permission_json) {
        Ok(b) => b,
        Err(e) => {
            log::warn!("[opencode] 序列化护栏配置失败: {e}");
            return OpenCodeOutcome::Failed {
                message: "护栏配置写入失败，已中止（拒绝在无护栏下执行）".into(),
            };
        }
    };
    if let Err(e) = std::fs::write(&config_path, config_bytes) {
        log::warn!("[opencode] 写护栏配置失败: {e}");
        return OpenCodeOutcome::Failed {
            message: "护栏配置写入失败，已中止（拒绝在无护栏下执行）".into(),
        };
    }

    let mut req = crate::opencode_agent::OpenCodeAgentRequest::new("opencode", prompt.to_string());
    req.cwd = cwd.map(|p| p.to_path_buf());
    req.model = model.map(|m| m.to_string());
    req.permission_mode = mode;
    // opencode 不支持 --settings / --tools CLI flags；权限配置通过 OPENCODE_CONFIG 环境变量
    // 传给子进程（见 mod.rs::run_opencode_agent，它会把 req.settings_json_path 转成 env var）。
    req.settings_json_path = Some(config_path.clone());
    ...
    let run = async_runtime::spawn(async move {
        crate::opencode_agent::run_opencode_agent("opencode", req, tx, cancel_for_runner).await
    });
    ...
    while let Some(ev) = rx.recv().await {
        use crate::opencode_agent::OpenCodeAgentEvent as E;
        match ev {
            E::Started { .. } => {
                emit_opencode(inner, serde_json::json!({ "kind": "started" }));
            }
            E::Delta { text, .. } => {
                emit_opencode(inner, serde_json::json!({ "kind": "delta", "text": text }));
            }
            E::ToolUse { name, .. } => {
                emit_opencode(inner, serde_json::json!({ "kind": "tool", "name": name }));
            }
            E::Completed {
                text, cost_usd: c, ..
            } => {
    ...
    if cancelled
        || matches!(
            &run_result,
            Ok(Err(crate::opencode_agent::OpenCodeAgentError::Cancelled))
        )
    {
        return OpenCodeOutcome::Cancelled;
    }

    let trimmed = final_text.trim().to_string();
    if !trimmed.is_empty() {
        OpenCodeOutcome::Done {
            text: trimmed,
            cost_usd,
        }
    } else {
        let message = error_msg
            ...
            .unwrap_or_else(|| "OpenCode 无结果（确认已登录 opencode 且额度充足）".to_string());
        OpenCodeOutcome::Failed { message }
    }
}
...
///
/// 无头 `opencode -p` 没有 mid-run 的 permission 通道，所以我们只能在「一轮跑完」后
/// 判断护栏是否拦了高风险动作：扫描终局文本里是否提到某个高风险模式 + 权限/拒绝/blocked
/// 关键词。命中则发 `approval` 事件、挂一个 oneshot 等用户决断（前端 Approve/Deny →
/// `opencode_approve` 命令解析）。
///
/// 返回 `Some(pattern)` 表示用户 Approve 了某高风险模式 → 调用方应放行该模式重跑一轮；
/// `None` 表示无需审批 / 用户 Deny / 超时。
async fn maybe_request_approval(
    inner: &Arc<Inner>,
    outcome: &OpenCodeOutcome,
) -> Option<String> {
    let text = match outcome {
        OpenCodeOutcome::Done { text, .. } => text.as_str(),
        OpenCodeOutcome::Failed { message } => message.as_str(),
        OpenCodeOutcome::Cancelled => return None,
    };
    ...
    let hit = crate::opencode_agent::guard::HIGH_RISK_PATTERNS
        .iter()
    ...
    if let Ok(mut map) = opencode_approvals().lock() {
...
fn opencode_agent_mode_from_pref(s: &str) -> crate::opencode_agent::OpenCodeAgentPermissionMode {
    use crate::opencode_agent::OpenCodeAgentPermissionMode as M;
    match s.trim() {
        "plan" => M::Plan,
        "default" => M::Default,
