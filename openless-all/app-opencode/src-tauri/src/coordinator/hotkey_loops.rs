...
pub(super) fn opencode_agent_hotkey_supervisor_loop(inner: Arc<Inner>) {
    // OpenCode (coding agent) is macOS-only. On Windows/Linux the binding can
    ...
    {
        update_opencode_agent_hotkey_binding_now(&inner);
    }
    #[cfg(target_os = "macos")]
    loop {
        if inner.shutdown.load(Ordering::SeqCst) {
...
pub(super) fn update_opencode_agent_hotkey_binding_now(inner: &Arc<Inner>) {
    #[cfg(not(target_os = "macos"))]
    {
        // OpenCode is intentionally macOS-only for now; keep Windows/Linux hidden and inert.
        take_opencode_agent_hotkeys_on_main_thread(inner);
        return;
    }
    ...
    {
        let prefs = inner.prefs.get();
        let Some(binding) = prefs.opencode_agent_voice_hotkey.clone() else {
            take_opencode_agent_hotkeys_on_main_thread(inner);
            log::info!("[opencode] hotkey disabled");
            return;
        };
        if !prefs.opencode_agent_enabled || is_unconfigured_shortcut(&binding) {
            take_opencode_agent_hotkeys_on_main_thread(inner);
            return;
        }

        if let Some(modifier_binding) = opencode_modifier_binding(&binding) {
            take_opencode_agent_combo_hotkey_on_main_thread(inner);
            if let Some(monitor) = inner.opencode_agent_modifier_hotkey.lock().as_ref() {
            ...
            match HotkeyMonitor::start(modifier_binding, tx) {
                Ok(monitor) => {
                    *inner.opencode_agent_modifier_hotkey.lock() = Some(monitor);
                    log::info!(
                        "[opencode] modifier hotkey installed ({})",
                        binding.display_label()
                    );
                    let bridge_inner = Arc::clone(inner);
                    std::thread::Builder::new()
                        .name("openless-opencode-modifier-bridge".into())
                        .spawn(move || opencode_modifier_bridge_loop(bridge_inner, rx))
                        .ok();
                }
                Err(e) => log::warn!("[opencode] modifier hotkey install failed: {e}"),
            }
            return;
        }

        inner.opencode_agent_modifier_hotkey.lock().take();
        let app = match inner.app.lock().clone() {
            Some(app) => app,
            None => {
                log::warn!("[opencode] AppHandle 未 bind，跳过组合键注册");
                return;
            }
        };
        let inner_clone = Arc::clone(inner);
        let binding_for_main = binding.clone();
        let _ = app.run_on_main_thread(move || {
            if let Some(monitor) = inner_clone.opencode_agent_combo_hotkey.lock().as_ref() {
                if let Err(e) = monitor.update_binding(binding_for_main.clone()) {
                    log::warn!("[opencode] combo hotkey update failed: {e}");
                }
                return;
            }
            ...
            match ComboHotkeyMonitor::start(binding_for_main.clone(), tx) {
                Ok(monitor) => {
                    *inner_clone.opencode_agent_combo_hotkey.lock() = Some(monitor);
                    log::info!(
                        "[opencode] combo hotkey installed ({})",
                        binding_for_main.display_label()
                    );
                    let bridge_inner = Arc::clone(&inner_clone);
                    std::thread::Builder::new()
                        .name("openless-opencode-combo-bridge".into())
                        .spawn(move || opencode_combo_bridge_loop(bridge_inner, rx))
                        .ok();
                }
                Err(e) => log::warn!("[opencode] combo hotkey install failed: {e}"),
            }
        });
    }
}
...
#[cfg(target_os = "macos")]
pub(super) fn opencode_modifier_binding(
...
pub(super) fn opencode_modifier_bridge_loop(inner: Arc<Inner>, rx: mpsc::Receiver<HotkeyEvent>) {
    while let Ok(evt) = rx.recv() {
        if inner.shortcut_recording_active.load(Ordering::SeqCst) {
            continue;
        }
        ...
        match evt {
            HotkeyEvent::Pressed => {
                async_runtime::block_on(async {
                    handle_opencode_pressed(&inner_cloned).await
                });
            }
            HotkeyEvent::Released => {
                async_runtime::block_on(async {
                    handle_opencode_released(&inner_cloned).await
                });
            }
...
pub(super) fn opencode_combo_bridge_loop(inner: Arc<Inner>, rx: mpsc::Receiver<ComboHotkeyEvent>) {
    while let Ok(evt) = rx.recv() {
        if inner.shortcut_recording_active.load(Ordering::SeqCst) {
            continue;
        }
...
pub(super) async fn handle_opencode_pressed(inner: &Arc<Inner>) {
    let prefs = inner.prefs.get();
    if !prefs.opencode_agent_enabled {
        return;
    }
    if !matches!(inner.state.lock().phase, SessionPhase::Idle) {
        log::info!("[opencode] press ignored: dictation session already active");
        return;
    }
    if !matches!(inner.qa_state.lock().phase, QaPhase::Idle) {
        log::info!("[opencode] press ignored: QA session active");
        return;
    }
    ...
    let started = {
        ...
        if matches!(
            ...
            log::info!(
                "[opencode] voice session started (session={:?})",
                state.session_id
            );
            true
        } else {
    ...
    if started {
        if let Some(app) = inner.app.lock().clone() {
            crate::show_opencode_glow(&app);
        }
    }
}

pub(super) async fn handle_opencode_released(inner: &Arc<Inner>) {
    ...
    match phase {
        ...
        SessionPhase::Starting => {
            ...
            request_stop_during_starting(inner, "opencode release edge");
...
pub(super) fn take_opencode_agent_hotkeys_on_main_thread(inner: &Arc<Inner>) {
    inner.opencode_agent_modifier_hotkey.lock().take();
    take_opencode_agent_combo_hotkey_on_main_thread(inner);
}

pub(super) fn take_opencode_agent_combo_hotkey_on_main_thread(inner: &Arc<Inner>) {
    let app = inner.app.lock().clone();
    if let Some(app) = app {
        let inner = Arc::clone(inner);
        let _ = app.run_on_main_thread(move || {
            inner.opencode_agent_combo_hotkey.lock().take();
        });
    } else {
        inner.opencode_agent_combo_hotkey.lock().take();
    }
