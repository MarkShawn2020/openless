...
macro_rules! app_invoke_handler_desktop {
    () => {
        tauri::generate_handler![
            ...
            opencode_agent::commands::opencode_agent_detect,
            opencode_agent::commands::opencode_agent_run,
            opencode_agent::commands::opencode_agent_cancel,
            opencode_agent::commands::opencode_agent_command_risk,
            ...
            commands::opencode_window_dismiss,
            commands::opencode_window_resize,
            commands::opencode_approve,
...
fn run_desktop() {
    ...
    builder
        ...
        .setup(move |app| {
            ...
            // QA / OpenCode / glow 浮窗改为懒创建（不再在 tauri.conf.json eager 声明）：
            // 用到时才 build（ensure_qa_window / ensure_opencode_window /
            // ensure_opencode_glow_window），idle 时根本没有它们的 WebKit 进程 ——
        ...
        .run(|app, event| match event {
            RunEvent::Ready => {
                ...
                coordinator.start_opencode_agent_hotkey_listener();
            ...
            RunEvent::Exit => {
                ...
                coordinator.stop_opencode_agent_hotkey_listener();
...
/// 懒创建 OpenCode 浮窗（macOS only）。
#[cfg(target_os = "macos")]
fn ensure_opencode_window<R: tauri::Runtime>(app: &AppHandle<R>) -> Option<tauri::WebviewWindow<R>> {
    if let Some(w) = app.get_webview_window("opencode") {
        return Some(w);
    }
    match WebviewWindowBuilder::new(
        app,
        "opencode",
        WebviewUrl::App("index.html?window=opencode".into()),
    )
    .title("OpenLess OpenCode")
    ...
    .build()
    {
        Ok(w) => Some(w),
        Err(e) => {
            log::warn!("[opencode] lazy window create failed: {e}");
            None
        }
    }
}

/// 懒创建 OpenCode glow 描边窗（macOS only）。shadow:false、无 acceptFirstMouse。
/// 它的 level/collectionBehavior/ignore-mouse 在每次 show_opencode_glow 里幂等设置，
/// 所以创建时不需要额外原生配置。
#[cfg(target_os = "macos")]
fn ensure_opencode_glow_window<R: tauri::Runtime>(
    app: &AppHandle<R>,
) -> Option<tauri::WebviewWindow<R>> {
    if let Some(w) = app.get_webview_window("opencode-glow") {
        return Some(w);
    }
    match WebviewWindowBuilder::new(
        app,
        "opencode-glow",
        WebviewUrl::App("index.html?window=opencode-glow".into()),
    )
    .title("OpenLess OpenCode Glow")
    ...
    .build()
    {
        Ok(w) => Some(w),
        Err(e) => {
            log::warn!("[opencode-glow] lazy window create failed: {e}");
            None
        }
    }
}

const OPENCODE_WINDOW_WIDTH: f64 = 400.0;
const OPENCODE_WINDOW_MIN_HEIGHT: f64 = 120.0;
const OPENCODE_WINDOW_MAX_HEIGHT: f64 = 520.0;

fn position_opencode_window<R: tauri::Runtime>(
    window: &tauri::WebviewWindow<R>,
    height: f64,
) -> tauri::Result<()> {
    let monitor = match window.current_monitor()? {
    ...
    let height = height.clamp(
        OPENCODE_WINDOW_MIN_HEIGHT,
        OPENCODE_WINDOW_MAX_HEIGHT,
    );
    let capsule_height = capsule_height_for_qa();
    let (x, y) = bottom_center_position(
        frame,
        OPENCODE_WINDOW_WIDTH,
        height,
        DOCK_BOTTOM_PADDING_FOR_QA + capsule_height + QA_WINDOW_GAP_TO_CAPSULE,
    );
    window.set_size(tauri::LogicalSize::new(OPENCODE_WINDOW_WIDTH, height))?;
    window.set_position(LogicalPosition::new(x, y))?;
    Ok(())
}

/// 显示 OpenCode 浮窗（不抢前台 app 焦点，与 QA 同手法）。`macos` 专用。
#[cfg(target_os = "macos")]
pub(crate) fn show_opencode_window<R: tauri::Runtime>(app: &AppHandle<R>) {
    let Some(window) = ensure_opencode_window(app) else {
        log::info!("[opencode] show 跳过：窗口不存在");
        return;
    };
    if let Err(e) = position_opencode_window(&window, OPENCODE_WINDOW_MIN_HEIGHT) {
        log::warn!("[opencode] position before show failed: {e}");
    }
    let window_clone = window.clone();
    let _ = app.run_on_main_thread(move || {
        use objc2::msg_send;
        ...
        match window_clone.ns_window() {
            Ok(handle) => {
                let ns = handle as *mut AnyObject;
                if ns.is_null() {
                    log::warn!("[opencode] ns_window null; falling back to window.show()");
                    let _ = window_clone.show();
                } else {
                    unsafe {
                        let _: () = msg_send![ns, orderFrontRegardless];
            ...
            Err(e) => {
                log::warn!("[opencode] ns_window unavailable: {e}; falling back to show()");
                let _ = window_clone.show();
            }
        }
    });
...
#[cfg(not(target_os = "macos"))]
pub(crate) fn show_opencode_window<R: tauri::Runtime>(_app: &AppHandle<R>) {}

/// 隐藏 OpenCode 浮窗。供 dismiss 命令 / session 收尾共用。
#[cfg(target_os = "macos")]
pub(crate) fn hide_opencode_window<R: tauri::Runtime>(app: &AppHandle<R>) {
...
#[cfg(not(target_os = "macos"))]
pub(crate) fn hide_opencode_window<R: tauri::Runtime>(_app: &AppHandle<R>) {}

/// 显示全屏彩虹描边浮层：盖满当前显示器、点击穿透、置顶。Agent 工作时点亮整屏边缘。
#[cfg(target_os = "macos")]
pub(crate) fn show_opencode_glow<R: tauri::Runtime>(app: &AppHandle<R>) {
    let Some(window) = ensure_opencode_glow_window(app) else {
        return;
    };
    let monitor = window
        .current_monitor()
    ...
    let _ = window.emit("opencode-glow:active", true);
    let window_clone = window.clone();
    let _ = app.run_on_main_thread(move || {
        use objc2::msg_send;
        use objc2::runtime::AnyObject;
...
#[cfg(not(target_os = "macos"))]
pub(crate) fn show_opencode_glow<R: tauri::Runtime>(_app: &AppHandle<R>) {}

/// 隐藏全屏彩虹描边浮层。
#[cfg(target_os = "macos")]
pub(crate) fn hide_opencode_glow<R: tauri::Runtime>(app: &AppHandle<R>) {
    if let Some(window) = app.get_webview_window("opencode-glow") {
        let _ = window.emit("opencode-glow:active", false);
        let _ = window.hide();
    }
}

#[cfg(not(target_os = "macos"))]
pub(crate) fn hide_opencode_glow<R: tauri::Runtime>(_app: &AppHandle<R>) {}

/// 前端按内容测高后回传。以「当前窗口底边」为锚向上生长——只改高度、保住用户拖动后的位置。
#[cfg(target_os = "macos")]
pub(crate) fn resize_opencode_window<R: tauri::Runtime>(app: &AppHandle<R>, height: f64) {
    let Some(window) = app.get_webview_window("opencode") else {
        return;
    };
    let height = height.clamp(
        OPENCODE_WINDOW_MIN_HEIGHT,
    ...
    match (window.outer_position(), window.outer_size()) {
        (Ok(pos), Ok(size)) => {
            ...
            let _ = window.set_size(tauri::LogicalSize::new(OPENCODE_WINDOW_WIDTH, height));
            let _ = window.set_position(LogicalPosition::new(x, new_y));
        }
        _ => {
            if let Err(e) = position_opencode_window(&window, height) {
                log::warn!("[opencode] resize fallback failed: {e}");
            }
        }
    }
}
...
#[cfg(not(target_os = "macos"))]
pub(crate) fn resize_opencode_window<R: tauri::Runtime>(_app: &AppHandle<R>, _height: f64) {}
