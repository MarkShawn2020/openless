...
struct Inner {
    ...
    opencode_agent_modifier_hotkey: Mutex<Option<HotkeyMonitor>>,
    opencode_agent_combo_hotkey: Mutex<Option<ComboHotkeyMonitor>>,
    ...
    /// OpenCode 连续对话：true=浮窗里已有进行中的会话，下一轮 `opencode --continue` 续上下文；
    /// 关闭浮窗（dismiss）复位为 false，下次说话开新会话。
    opencode_conversation: AtomicBool,
}
...
impl Coordinator {
        ...
        #[cfg(not(target_os = "windows"))]
        {
            ...
            Self {
                inner: Arc::new(Inner {
                    ...
                    opencode_agent_modifier_hotkey: Mutex::new(None),
                    opencode_agent_combo_hotkey: Mutex::new(None),
                    ...
                    opencode_conversation: AtomicBool::new(false),
                }),
            }
        }
    }
    ...
    /// 仅在 `opencode_agent_enabled` 时注册。
    pub fn start_opencode_agent_hotkey_listener(&self) {
        let inner = Arc::clone(&self.inner);
        std::thread::Builder::new()
            .name("openless-opencode-agent-hotkey-supervisor".into())
            .spawn(move || opencode_agent_hotkey_supervisor_loop(inner))
            .ok();
    }

    pub fn stop_opencode_agent_hotkey_listener(&self) {
        take_opencode_agent_hotkeys_on_main_thread(&self.inner);
    }

    pub fn update_opencode_agent_hotkey_binding(&self) {
        update_opencode_agent_hotkey_binding_now(&self.inner);
    }
    ...
    /// 用户点 ✕ / 按 Esc 关 OpenCode 浮窗：隐藏窗口 + 结束连续对话
    /// （下次说话开新会话，不再 --continue 续旧上下文）。
    pub fn opencode_window_dismiss(&self) {
        self.inner
            .opencode_conversation
            .store(false, Ordering::SeqCst);
        if let Some(app) = self.inner.app.lock().clone() {
            crate::hide_opencode_window(&app);
            crate::hide_opencode_glow(&app);
        }
    }

    /// 前端按内容测高后回传，后端 clamp + bottom-anchored 重新摆放 OpenCode 浮窗。
    pub fn opencode_window_resize(&self, height: f64) {
        if let Some(app) = self.inner.app.lock().clone() {
            crate::resize_opencode_window(&app, height);
        }
    }

    /// 内联审批卡的 Approve / Deny 回执：解析等待中的 token。
    pub fn opencode_approve(&self, token: &str, approved: bool) {
        dictation::resolve_opencode_approval(token, approved);
    }
...
mod tests {
    ...
    /// 复现并验证目标 2(a)：按下 OpenCode 键必须弹出可见胶囊。
    /// 这里直接驱动 bridge 会调用的 handler，断言 begin_session 确实下发了可见胶囊。
    #[tokio::test]
    async fn opencode_press_emits_visible_capsule() {
        let _guard = ENV_LOCK.lock().await;
        std::env::set_var("OPENLESS_HOTKEY_INJECTION_DRY_RUN", "1");
        ...
        let coordinator = Coordinator::new();
        {
            ...
            prefs.opencode_agent_enabled = true;
            coordinator.inner.prefs.set(prefs).unwrap();
        }
        // 前置：还没弹过任何胶囊。
        assert!(coordinator.inner.last_capsule_state.lock().is_none());

        // 等价于「按下 OpenCode 键」：bridge_loop 收到 Pressed 后就是调这个 handler。
        super::handle_opencode_pressed(&coordinator.inner).await;

        assert_eq!(
            *coordinator.inner.last_capsule_state.lock(),
            Some(CapsuleState::Recording),
            "按下 OpenCode 键必须进入录音并弹出可见胶囊"
        );
        std::env::remove_var("OPENLESS_HOTKEY_INJECTION_DRY_RUN");
    }
