// OpenCodePanel.tsx — OpenCode 语音 Agent 浮窗（窗口 label = "opencode"）。
//
...
// 仅 macOS 实际触发（后端只在 macOS 注册 OpenCode 热键并 emit 事件）。
// 关闭：Esc / ✕ → opencode_window_dismiss → 后端隐藏窗口。
...
import {
  isTauri,
  opencodeApprove,
  opencodeWindowDismiss,
  opencodeWindowResize,
} from '../lib/ipc';
import type { OpenCodeEvent } from '../lib/types';
...
export function OpenCodePanel() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const handle = await listen<OpenCodeEvent>(
          'opencode:event',
        ...
        console.error('[OpenCode] listener setup failed', error);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const applyEvent = (ev: OpenCodeEvent) => {
    switch (ev.kind) {
      case 'user': {
  ...
  const onApproval = (token: string, approved: boolean) => {
    ...
    void opencodeApprove(token, approved);
  };

  const onClose = () => void opencodeWindowDismiss();
  ...
  useEffect(() => {
    ...
    resizeFrame.current = requestAnimationFrame(() => {
      ...
      void opencodeWindowResize(target);
    });
  }, [turns]);
  useEffect(
    () => () => {
  ...
  return (
    <div className="ol-frost lc-shell" style={shellStyle}>
      ...
      <Toolbar label={t('opencode.closeTooltip')} onClose={onClose} />
...
function TurnView({
  ...
  return (
    <>
      <UserBubble text={turn.user} label={t('opencode.you')} />
      ...
      {turn.status === 'working' && !turn.answer && <WorkingRow label={t('opencode.working')} />}
      {turn.status === 'error' && <ErrorRow message={turn.errorMsg || t('opencode.error')} />}
      {turn.status === 'cancelled' && <CostRow label={t('common.cancelled')} />}
      {turn.status === 'done' && turn.costUsd != null && (
        <CostRow label={t('opencode.cost', { cost: turn.costUsd.toFixed(3) })} />
      )}
    </>
  );
}
...
function AssistantBubble({ markdown, working }: { markdown: string; working: boolean }) {
  ...
  const html = useMemo(() => {
    ...
    try {
      ...
      console.error('[OpenCode] markdown render failed', error);
    ...
    <div className="lc-enter" style={approvalCardStyle}>
      ...
      {!decided && (
        <div style={approvalRerunWarningStyle}>
          {t('opencode.approvalRerunWarning')}
        </div>
      )}
      {decided ? (
...
if (typeof document !== 'undefined' && !document.getElementById('opencode-panel-style')) {
  const tag = document.createElement('style');
  tag.id = 'opencode-panel-style';
