// OpenCode 全屏彩虹边缘亮条（独立窗口 window=opencode-glow）。
...
if (typeof document !== 'undefined' && !document.getElementById('opencode-glow-style')) {
  const tag = document.createElement('style');
  tag.id = 'opencode-glow-style';
  tag.textContent = glowCss;
  document.head.appendChild(tag);
}

export function OpenCodeGlow() {
  ...
  useEffect(() => {
    ...
    void import('@tauri-apps/api/event').then(({ listen }) => {
      if (cancelled) return;
      void listen<boolean>('opencode-glow:active', (e) => {
