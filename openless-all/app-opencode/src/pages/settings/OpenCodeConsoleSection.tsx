// 高级 → OpenCode 控制台：检测 opencode 安装状态，
// 并护栏化地无头跑一次 opencode、流式查看输出与用量。这是「快速 Agent」引擎的
// 最小可用垂直切片，不依赖录音 / coordinator。

import { useEffect, useRef, useState, type CSSProperties } from 'react'
import { useTranslation } from 'react-i18next'
import {
  opencodeAgentCancelTest,
  opencodeAgentCommandRisk,
  opencodeAgentDetect,
  opencodeAgentRunTest,
  isTauri,
  type CommandRisk,
  type OpenCodeAgentEvent,
  type OpenCodeAgentPermissionMode,
...
export function OpenCodeConsoleSection() {
  const { t } = useTranslation()
  const [detection, setDetection] = useState<OpenCodeDetection | null>(null)
  const [detecting, setDetecting] = useState(false)
  const [workdir, setWorkdir] = useState('')
  const [permMode, setPermMode] = useState<OpenCodeAgentPermissionMode>('acceptEdits')
  const [prompt, setPrompt] = useState('')
  const [running, setRunning] = useState(false)
  const [output, setOutput] = useState('')
  const [summary, setSummary] = useState<string | null>(null)
  const [risk, setRisk] = useState<CommandRisk | null>(null)
  ...
  async function runDetect() {
    setDetecting(true)
    try {
      setDetection(await opencodeAgentDetect())
    } finally {
      setDetecting(false)
    }
  }
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const un = await listen<OpenCodeAgentEvent>('opencode-agent:test', e => {
        const ev = e.payload
        switch (ev.kind) {
          case 'started':
            setOutput('')
  ...
  const onRun = async () => {
    const p = prompt.trim()
    if (!p || running) return
    setRisk(await opencodeAgentCommandRisk(p))
    setRunning(true)
    setOutput('')
    setSummary(null)
    try {
      await opencodeAgentRunTest({
        prompt: p,
  ...
  return (
    <Card>
      ...
      <SettingRow label={t('settings.codingConsole.permissionMode')}>
        <select
          value={permMode}
          onChange={e => setPermMode(e.target.value as OpenCodeAgentPermissionMode)}
      ...
      <div style={{ paddingTop: 14, borderTop: '0.5px solid var(--ol-line-soft)' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 10, alignItems: 'center' }}>
          ...
          {running && (
            <Btn variant="ghost" size="sm" onClick={() => void opencodeAgentCancelTest()}>
