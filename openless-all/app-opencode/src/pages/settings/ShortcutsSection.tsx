...
import { defaultOpenCodeShortcut, defaultOpenAppShortcut, defaultQaShortcut, defaultSwitchStyleShortcut } from '../../lib/hotkey';
import {
  setDictationHotkey,
  setOpenAppHotkey,
  setQaHotkey,
...
export function ShortcutsSection() {
  ...
  return (
    <Card>
      ...
      {os === 'mac' && (
        <SettingRow label={t('settings.opencodeAgent.title')} desc={t('settings.opencodeAgent.voiceHotkeyDesc')}>
          {prefs.opencodeAgentEnabled && prefs.opencodeAgentVoiceHotkey ? (
            <ShortcutRecorder
              value={prefs.opencodeAgentVoiceHotkey}
              alignRecordButton
              onSave={async binding => {
                await savePrefs({ ...prefs, opencodeAgentVoiceHotkey: binding });
              }}
              onDisable={async () => {
                await savePrefs({ ...prefs, opencodeAgentVoiceHotkey: null });
              }}
            />
          ) : (
            <button
              onClick={() =>
                void savePrefs({
                  ...prefs,
                  opencodeAgentEnabled: true,
                  opencodeAgentVoiceHotkey: prefs.opencodeAgentVoiceHotkey ?? defaultOpenCodeShortcut(),
                })
              }
              style={enableBtnStyle}
            >
