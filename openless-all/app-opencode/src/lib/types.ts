...
export type OpenCodeAgentProviderId = "claude-code-cli" | "opencode-cli";
...
export interface UserPreferences {
  ...
  opencodeAgentEnabled: boolean;
  /** Agent 后端：claude-code-cli（默认）/ opencode-cli。 */
  opencodeAgentProvider: OpenCodeAgentProviderId;
  /** Agent 模型，null = 运行时取便宜默认（sonnet）。 */
  opencodeAgentModel: string | null;
  /** 权限模式：plan/default/acceptEdits/bypassPermissions。 */
  opencodeAgentPermissionMode: OpenCodeAgentPermissionMode;
  /** Agent 工作目录，null = 临时目录。 */
  opencodeAgentWorkdir: string | null;
  /** Less Computer 按住说话快捷键。null = 停用；目前仅 macOS 显示/生效。 */
  opencodeAgentVoiceHotkey: ShortcutBinding | null;
  /** 热键 1：语音 Agent 面板键。null = 停用。 */
  opencodeAgentPanelHotkey: ShortcutBinding | null;
  /** 热键 2：快取用键（选中→Claude→回插）。null = 未配置。 */
  opencodeAgentQuickHotkey: ShortcutBinding | null;
...
/**
 * Less Computer 语音 Agent 浮窗事件（窗口 label = "opencode"，事件名
 * `opencode:event`）。后端按 `kind` 标记，前端据此把交互渲染成聊天结构。
 */
export type OpenCodeEvent =
  ...
  /** Agent 启动，进入运行态。 */
  | { kind: 'started' }
  /** 流式回复增量（来自 OpenCodeAgentEvent::Delta）。 */
  | { kind: 'delta'; text: string }
  /** 工具调用提示（来自 OpenCodeAgentEvent::ToolUse，如 "Bash"）。 */
