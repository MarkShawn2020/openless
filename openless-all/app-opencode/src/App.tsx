...
const OpenCodePanel = lazy(() =>
  import('./pages/OpenCodePanel').then(m => ({ default: m.OpenCodePanel })),
);
const OpenCodeGlow = lazy(() =>
  import('./pages/OpenCodeGlow').then(m => ({ default: m.OpenCodeGlow })),
);
...
export function App({ isCapsule, isQa, isOpenCode, isOpenCodeGlow, forcedOs }: AppProps) {
  if (isCapsule) {
    return <Capsule />;
  }
  if (isQa) {
